...
static int
_InUse(_UsedAndSize uas)
{
  return (uas & IN_USE_FLAG) != 0;
}

static void
_SetInUse(_UsedAndSize* uas)
{
  *uas |= IN_USE_FLAG;
}

static void
_SetFree(_UsedAndSize* uas)
{
  *uas &= ~IN_USE_FLAG;
}

static Size
_GetSize(_UsedAndSize uas)
{
  return uas & ~IN_USE_FLAG;
}

static void
_SetSize(_UsedAndSize* uas, Size size)
{
  assert(!_InUse(*uas));
...
static _Frag*
_GetLargerNeighbour(_Frag* frag)
{
...
static _Frag*
_GetSmallerNeighbour(_Frag* frag)
{
...
static _Frag*
_SplitFrag(_Frag* frag, Size pos)
{
...
static void
_RemoveFrag(_Frag* frag, _MemoryImpl* mem)
{
  if (frag->prev == NULL) {
...
*/
static int
largebin_index(unsigned int sz)
{
...
static unsigned int
_IndexBin(Size fragSize)
{
  return bin_index(fragSize);
}

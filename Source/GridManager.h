...
// abort on error
void
InitManager(GridManager* manager);
void
FinalizeManager(
  GridManager* manager); // notice: this function does not free memory
...
Grid*
AllocGrid(GridManager* manager, Size size);
void*
FreeGrid(GridManager* manager, Grid* grid);

// indicate that the following processing exclusively writes to the specific
// grid notice that maybe more actions are required to implement full protection
// (such as modify .so) just do whatever need to do here
void
SetCurrentGrid(GridManager* manager, Grid* grid);
